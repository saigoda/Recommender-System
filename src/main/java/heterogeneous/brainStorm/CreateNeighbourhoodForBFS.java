 ...
 * 
 * Prepares input for the BFS algorithm. Takes as input the concatenated network.
 *
 */
public class CreateNeighbourhoodForBFS extends Configured implements Tool {
	...
	public static class CreateNeighbourhoodForBFSMap extends MapReduceBase
		 ...
		 * 
		 * Key-- UserID:itemsClicked (Ex:10000_user:30) 
		 * Value--List of neighbors(Ex:2345_user:30:<dist>,3456_user:40:<dist>....)
		 */
	...
	public static class CreateNeighbourhoodForBFSReduce extends MapReduceBase
		...
		/**
		 * 
		 *  **TODO - Change the way in which the source node is selected. Some possible variations
		 *  1. Node which clicked most items
		 *  2. Node with highest authority score
		 *  3. Node with highest hub score.
		 *  **
		 ...
		 *  Value - neighborhood info with commonItems represented as distribution
		 * 
		 * For source node the output is:
		 * 
		 * <Key>\t<neighbor1>,<neighbor2>,<neighbor3>,<neighbor4>|distance|GRAY|source|1 (last field is the score)
		 * 
		 * The distance is the distance from
		 * the parent node. If the distance is modified then it means the parent
		 * of the node changed. The source node is GRAY indicating that this is
		 * the node from which the processing should start. The 'source' indicates
		 ...
		 * 
		 *##################
		 *At each node, the last integer indicates the score value for that node. This score is used to compute the score for its children while doing
		 *BFS
		 *##################
		 */
		public void reduce(Text key, Iterator<Text> values,
			...
			if (!source) {
				...
				output.collect(key, new Text(b.toString()+ "|"+Integer.MAX_VALUE+"|WHITE|null|0"));
		}
	...
	public static void main(String[] args) {
		try {
			ToolRunner.run(new Configuration(),new CreateNeighbourhoodForBFS(), args);
		} catch (Exception e) {
	...
	public int run(String[] args) throws Exception {
		...
		/*NumReducers should always be 1*/
