...
public class ConcatenateTwoNetworks extends Configured implements Tool {
	...
	public static class ConcatenateTwoNetworksMap extends MapReduceBase
		...
		public void map(LongWritable key, Text value,
			...
			output.collect(new Text(splits[0].split(":")[0].trim()), new Text(
					splits[1]));
		}
	}
	...
	public static class ConcatenateTwoNetworksReduce extends MapReduceBase
		...
		public void reduce(Text key, Iterator<Text> values,
			...
			for (String s : buffer.toString().split(",")) {
				s = s.split(":")[0];
				//If the neighbor is already in the hash set increment it's count. Otherwise we store it in the hashset
	...
	public static void main(String[] args) {
		try {
			ToolRunner.run(new Configuration(), new ConcatenateTwoNetworks(),
					args);
		} catch (Exception e) {
	...
	public int run(String[] args) throws Exception {
		...
		job1.setJobPriority(JobPriority.VERY_HIGH);
